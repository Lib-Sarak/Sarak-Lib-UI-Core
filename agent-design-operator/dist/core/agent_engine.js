...
export class AgentEngine {
    db = databaseClient;
    async _ensureKnowledgeIndexed(agentId, tableKnowledge, rawKnowledge, embeddingsProvider, tableState, vectorDb) {
    ...
    async processMessage(request) {
        ...
        let config, identity, knowledge, workflow, rules, manifest;
        try {
        ...
        if (useRag) {
            ...
            this._ensureKnowledgeIndexed(agentId, tableKnowledge, knowledge, activeEmbeddings, tableState, vectorStore).catch((err) => {
            ...
            try {
                ...
                const matches = await vectorStore.similaritySearch(tableKnowledge, agentId, queryVector, topK, threshold);
        ...
        const systemPrompt = `${GLOBAL_SYSTEM_CONSTRAINTS}\n\n` +
            `[AGENT IDENTITY]\n${identity}\n\n` +
            `${sessionStateText}` +
        ...
        for (const attempt of modelsToTry) {
            try {
                ...
                while (attemptCount < maxAttempts && !success) {
                    try {
                        attemptCount++;
                        rawResponse = await provider.generateResponse(systemPrompt, formattedHistory, llmTemperature, llmMaxTokens, attempt.model);
                        success = true;
                    }
                    catch (netErr) {
