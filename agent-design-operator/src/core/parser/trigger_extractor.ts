import { TriggerAction } from '../../config/shared/types.js';
import { logger } from '../../utils/logger.js';

export class TriggerExtractor {
    static extractTriggers(rawText: string, triggersConfig?: Record<string, any>): [string, TriggerAction[]] {
        ...
        for (const [triggerName, info] of Object.entries(triggersConfig)) {
            ...
            const fields: string[] = info.fields || [];
            ...
            try {
                ...
                for (const m of matches) {
                    ...
                    logger.info(`Extracted dynamic [${triggerName}] trigger: ${JSON.stringify(data)}`);
                    actions.push({
                        type: triggerName,
                        data: data
                    });
                ...
                logger.error(`Error parsing dynamic trigger '${triggerName}' with pattern '${patternStr}': ${e.message}`);
            }
        }
