...
0. **Regra Zero Absoluta (Design as Data):**
   - O ecossistema inteiro é orientado a dados. Se o usuário pedir para alterar um layout, criar um preset ou montar um tema, você deve entender que isso se resume **EXCLUSIVAMENTE** a preencher os valores das propriedades mapeadas no Payload/JSON do componente. **É estritamente proibido** tentar alterar a estrutura de arquivos `.tsx` ou injetar novo CSS genérico para satisfazer um ajuste de design. O layout muda através dos dados preenchidos.
...
1. **Manifestos Arquiteturais (A Fundação):**
   ...
   - `specs/specs/09-expansao-vs-configuracao.md`: A fronteira estrita entre Configurar via JSON e Expandir via Engenharia TS.
