import { ThemePreset } from './index';

export const neobrutalismTheme: ThemePreset = {
    id: 'neo-brutalism',
    name: 'Neo Brutalism',
    description: 'Aggressive contrast, thick white borders on near-black cards, solid shadows, flat geometry.',
    design: {
        tooltipTextColor: '#6280c7',
        mode: "dark",
        navigationStyle: "topbar",
        bodySize: "14px",
        iconFamily: 'tabler',
        iconWeight: 'bold',        identityAlignment: "left",
        identityPadding: 16,
        identityFontFamily: "Inter",
        identityFontWeight: 700,
        identityTracking: 0,
        identityRedirectUrl: "/",
        identityHoverEffect: "opacity",
        bgBaseColor: "#f0f0f0",
        layout: "grid",
        layoutDensity: "comfortable",
        maxContentWidth: "1440px",
        layoutPadding: 32,
        isSplitViewEnabled: false,
        isAutoHideEnabled: false,
        borderRadius: 12,
        borderRadiusSm: 6,
        borderRadiusMd: 12,
        borderRadiusLg: 20,
        borderWidth: 1,
        borderStyle: "solid",
        layoutGap: 24,
        layoutGapSm: 12,
        layoutGapMd: 24,
        layoutGapLg: 36,
        iconStrokeWidth: 2,
        scrollbarWidth: 6,
        scrollbarThumbColor: "rgba(255, 255, 255, 0.1)",
        industrialRegistry: true,
        isNavHidden: false,
        sidebarWidth: 240,
        sidebarMinWidth: 200,
        sidebarMaxWidth: 450,
        sidebarColor: "#000000",
        sidebarHoverColor: "rgba(255, 0, 0, 0.12)",
        sidebarActiveColor: "transparent",
        sidebarNoiseOpacity: 0,
        topbarHeight: 64,
        topbarColor: "#000000",
        topbarHoverColor: "rgba(255, 0, 0, 0.12)",
        topbarActiveColor: "transparent",
        topbarNoiseOpacity: 0,
        topbarTitleColor: "#ffffff",
        tabGap: 8,
        tabSectionMargin: 16,
        navItemActiveColor: "#ff0000",
        navActiveMarkerColor: "#ff0000",
        navActiveMarkerGlow: 10,
        sidebarBlur: 10,
        sidebarShadow: "10px 0 30px rgba(0,0,0,0.5)",
        cardBorderRadius: 4,
        cardRadiusTL: 12,
        cardRadiusTR: 12,
        cardRadiusBL: 12,
        cardRadiusBR: 12,
        cardGeometricCut: 0,
        cardBackgroundColor: "#0a0a0a",
        cardBackdropBlur: 12,
        cardSurfaceOpacity: 0.8,
        cardBorderWidth: 4,
        cardBorderColor: "#ffffff",
        cardBorderOpacity: 1,
        cardBorderTop: 1,
        cardBorderBottom: 1,
        cardBorderLeft: 1,
        cardBorderRight: 1,
        cardInnerGlowColor: "rgba(255, 255, 255, 0.05)",
        cardInnerGlowWidth: 1,
        cardTextureType: "none",
        cardTextureOpacity: 0.03,
        cardHeaderBg: "rgba(255, 255, 255, 0.03)",
        cardHeaderBorder: "rgba(255, 255, 255, 0.05)",
        cardHeaderPadding: 16,
        cardFooterBg: "transparent",
        cardFooterBorder: "rgba(255, 255, 255, 0.05)",
        cardShadowSpread: 0,
        cardGlowColor: "rgba(0, 242, 255, 0.05)",
        cardGlowIntensity: 0,
        cardHoverTranslate: -4,
        cardHoverGlowIncrease: 0.2,
        cardSpotlightOpacity: 0.1,
        cardPaddingMd: 24,
        cardVariant: "classic",
        cardTitleFontSize: 20,
        cardTitleColor: "#ffffff",
        cardTitleFontWeight: "900",
        cardTitleLetterSpacing: 0,
        cardTitleIconGlow: "rgba(0, 242, 255, 0.2)",
        cardActionBtnPrimaryBg: "#cc0000",
        cardActionBtnHoverBg: "#a30000",
        cardActionBtnText: "#ffffff",
        cardActionBtnBorderRadius: 8,
        cardActionBtnPadding: 12,
        cardActionClickScale: 0.96,
        cardSearchBgFocus: "rgba(0, 242, 255, 0.08)",
        cardSearchBorderBeamActive: true,
        cardSearchPlaceholderColor: "rgba(255, 255, 255, 0.4)",
        cardSearchTextFocusColor: "#ffffff",
        modalOverlayColor: "rgba(0, 0, 0, 0.4)",
        modalOverlayBlur: 8,
        modalBorderRadius: 16,
        tooltipBg: "#0f172a",
        tooltipRadius: 4,
        tableHeaderBg: "rgba(255, 255, 255, 0.03)",
        tableRowHoverBg: "rgba(255, 255, 255, 0.02)",
        tableCellPadding: 12,
        tableBorderColor: "rgba(255, 255, 255, 0.05)",
        headingFont: "\"Space Grotesk\", sans-serif",
        bodyFont: "\"Inter\", sans-serif",
        monoFont: "'JetBrains Mono', monospace",
        textColorMaster: '#838383',
        textColorSecondary: "rgba(255, 255, 255, 0.7)",
        textColorMuted: '#ffffff77',
        h1Size: 48,
        h1Weight: "900",
        h1LineHeight: 1.1,
        h1LetterSpacing: -1,
        h2Size: 32,
        h2Weight: "700",
        h2LineHeight: 1.2,
        bodyLineHeight: 1.6,
        bodyWeight: "400",
        headingTransform: "none",
        textSmoothing: true,
        textGlowIntensity: 0,
        btnBorderRadius: 4,
        btnRadiusTL: 8,
        btnRadiusTR: 8,
        btnRadiusBL: 8,
        btnRadiusBR: 8,
        btnHoverScale: 1.02,
        btnActiveScale: 0.98,
        btnPrimaryBg: "#ff0000",
        btnPrimaryText: '#151515',
        btnSecondaryBg: "rgba(255, 255, 255, 0.05)",
        btnGhostHoverBg: "rgba(255, 255, 255, 0.05)",
        btnStyleType: "matte",
        btnNeonGlowColor: "rgba(0, 242, 255, 0.4)",
        btnNeonPulseSpeed: 1.5,
        btnBackdropBlur: 8,
        inputBorderRadius: 4,
        inputBg: "rgba(255, 255, 255, 0.03)",
        switchTrackActiveBg: "#00f2ff",
        switchThumbBg: "#ffffff",
        checkboxActiveColor: "#00f2ff",
        switchStyleType: "tactile",
        switchBackdropBlur: 4,
        switchPulseColor: "#00f2ff",
        colorPalette: "default",
        primaryColor: "#ff0000",
        secondaryColor: "#ffff00",
        tertiaryColor: "#10b981",
        accentColor: "#0000ff",
        surfaceColor: "#0f0f0f",
        textureColor: "#ffffff",
        titleColor: "#ffffff",
        colorDepth: 1,
        colorVariation: 1,
        colorBgBody: "#050505",
        colorBgLayer1: "#0f0f0f",
        colorBgLayer2: "#1a1a1a",
        colorBgModal: "rgba(15, 15, 15, 0.8)",
        chartColorPalette: "#00f2ff",
        chartGridOpacity: 0.05,
        chartTooltipBg: "rgba(15, 23, 42, 0.9)",
        chartType: "line",
        chartShowGrid: true,
        chartThickness: 2,
        chartSmoothing: true,
        animInstant: 100,
        animFast: 200,
        animNormal: 300,
        animSlow: 500,
        easeMain: "cubic-bezier(0.4, 0, 0.2, 1)",
        easeOut: "cubic-bezier(0, 0, 0.2, 1)",
        animEnabled: true,
        bgGradientMode: "linear",
        bgGradientAngle: 135,
        surfaceMaterial: "frosted",
        surfaceIntensity: 0.5,
        borderType: "solid",
        systemTone: "dark",
        texture: "grid",
        textureOpacity: 0.1,
        atmosphereNoiseOpacity: 0.05,
        noiseIntensity: 0.05,
        bgNoiseDensity: 0.05,
        bgNoiseAnimation: 1,
        glassBlur: 0,
        glassOpacity: 0.1,
        glassSpecularity: 0.1,
        glassRoughness: 0.05,
        glassSaturation: 1.2,
        shadowIntensity: 1,
        layeredShadows: 1,
        shadowOrientation: "bottom",
        shadowColorMode: "neutral",
        shadowAmbientAlpha: 0.4,
        shadowProjectionBlur: 40,
        shadowProjectionAlpha: 0.2,
        securityShieldGlow: 10,
        securityPulseSpeed: 2,
        vignetteOpacity: 0.3,
        vignetteSoftness: 50,
        globalSaturation: 1,
        globalContrast: 1,
        contrastCurve: 1,
        chatBubbleRadius: 12,
        chatUserBg: "rgba(255, 255, 255, 0.05)",
        chatBubbleStyle: "glass",
        chatAnimationSpeed: 0.05,
        statusSuccessColor: "#10b981",
        statusErrorColor: "#ef4444",
        statusWarningColor: "#f59e0b",
        statusInfoColor: "#3b82f6",
        badgeRadius: 99,
        focusRingWidth: 2,
        reducedMotion: false,
        zIndexModal: 2000,
        zIndexToast: 2000,
        aiPanelBg: "rgba(0, 0, 0, 0.4)",
        aiGlowColor: "#00f2ff",
        flowNodeRadius: 12,
        flowGridStyle: "dots",
        motionEaseMain: "cubic-bezier(0.4, 0, 0.2, 1)",
        motionEaseOut: "cubic-bezier(0, 0, 0.2, 1)",
        motionEaseIn: "cubic-bezier(0.4, 0, 1, 1)",
        motionDurationInstant: 0.1,
        motionDurationFast: 0.2,
        motionDurationNormal: 0.3,
        motionDurationSlow: 0.6,
        motionStaggerDelay: 0.05,
        scrollWidth: 6,
        scrollRadius: 10,
        scrollTrackOpacity: 0.05,
        scrollThumbColor: "#ffffff",
        scrollThumbOpacity: 0.2,
        scrollThumbHoverOpacity: 0.5,
        scrollPadding: 2,
        zIndexBase: 1,
        zIndexSidebar: 500,
        zIndexTooltip: 9000,
        layerBackdropBlur: 10,
        layerBackdropOpacity: 0.4,
        layerElevationFactor: 1,
        matrixGap: 12,
        matrixRadius: 12,
        matrixItemBg: "rgba(255, 255, 255, 0.02)",
        matrixBorderColor: "rgba(255, 255, 255, 0.05)",
        matrixSearchBg: "rgba(255, 255, 255, 0.05)",
        matrixBlur: 10,
        cardShadow: "8px 8px 0px #000000",
        enabledLanguages: ['pt', 'en', 'es', 'fr', 'de', 'it'],
    },
    contraparte: {
        mode: 'light',
        bgBaseColor: '#ffffff',
        sidebarColor: '#ffffff',
        sidebarHoverColor: 'rgba(255, 0, 0, 0.08)',
        topbarColor: '#ffffff',
        topbarHoverColor: 'rgba(255, 0, 0, 0.08)',
        topbarTitleColor: '#0a0a0a',
        cardBackgroundColor: '#ffffff',
        cardBorderColor: '#000000',
        cardHeaderBg: '#f5f5f5',
        cardHeaderBorder: '#000000',
        cardFooterBg: 'transparent',
        cardFooterBorder: '#000000',
        cardTitleColor: '#0a0a0a',
        cardActionBtnText: '#ffffff',
        cardSearchBgFocus: '#f5f5f5',
        cardSearchPlaceholderColor: '#6b6b6b',
        cardSearchTextFocusColor: '#0a0a0a',
        modalOverlayColor: 'rgba(255, 255, 255, 0.85)',
        tooltipBg: '#0a0a0a',
        tooltipTextColor: '#ffffff',
        tableHeaderBg: '#f5f5f5',
        tableRowHoverBg: '#f0f0f0',
        tableBorderColor: '#000000',
        textColorMaster: '#0a0a0a',
        textColorSecondary: '#3a3a3a',
        textColorMuted: '#6b6b6b',
        btnSecondaryBg: '#f0f0f0',
        btnGhostHoverBg: '#f5f5f5',
        inputBg: '#ffffff',
        inputTextColor: '#0a0a0a',
        surfaceColor: '#f5f5f5',
        navItemActiveColor: '#cc0000',
        titleColor: '#0a0a0a',
        colorBgBody: '#ffffff',
        colorBgLayer1: '#f5f5f5',
        colorBgLayer2: '#efefef',
        colorBgModal: '#ffffff',
        chartTooltipBg: '#f5f5f5',
        scrollThumbColor: '#0a0a0a',
        matrixItemBg: '#f5f5f5',
        matrixBorderColor: '#000000',
        matrixSearchBg: '#ffffff',
    }
};
