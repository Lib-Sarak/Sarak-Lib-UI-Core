...
interface ThemeEditorProps {
    ...
    setPreviewEmojiSet: (set: string) => void;
    openSections: string[];
    onToggleSection: (id: string) => void;
    primaryColors: { name: string, value: string }[];
    previewPrimaryColor: string;
    setPreviewPrimaryColor: (color: string) => void;
    previewFontScale: string;
    setPreviewFontScale: (scale: string) => void;
}
...
export const ThemeEditor: React.FC<ThemeEditorProps> = ({
    ...
    return (
        <div className="flex-grow overflow-y-auto custom-scrollbar border-t border-[var(--theme-border)]/50 bg-black/5">
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="relative">
                            ...
                            />
                            <div className="w-6 h-6 rounded-full border border-[var(--theme-border)]/50" style={{ backgroundColor: previewPrimaryColor }}></div>
                        </div>
                    </div>
                    <div className="grid grid-cols-7 gap-2">
                        {primaryColors.map((color, i) => (
                            <button
                                ...
                                className={`w-full aspect-square rounded-full transition-all hover:scale-110 ${previewPrimaryColor === color.value ? 'ring-2 ring-[var(--theme-primary)] ring-offset-2 ring-offset-[var(--theme-card)]' : 'opacity-80'}`}
