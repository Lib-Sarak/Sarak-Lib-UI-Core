        ...
        <>
            ...
            <Section id="font-refinement" icon={Type} title="Refinamento Técnico" activeSection={activeSection} onToggle={setActiveSection}>
                <div className="flex flex-col gap-2 mb-4">
                    <span className="text-3xs font-black uppercase tracking-widest text-white/40">Tamanho Global (Escala)</span>
                    <div className="flex p-1 bg-white/5 rounded-xl border border-white/5">
                        {['pp', 'p', 'm', 'g', 'gg'].map(s => (
                            <button key={s} onClick={() => updateDraft('fontScale', s)} className={`flex-1 py-1.5 rounded-lg text-3xs font-black uppercase transition-all ${draft.fontScale === s ? 'bg-[var(--theme-primary)] text-white' : 'text-white/20 hover:text-white/40'}`}>
                                {s}
                            </button>
                        ))}
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-4">
                    ...
                    <div className="flex flex-col gap-1">
                        <span className="text-3xs font-black uppercase tracking-widest text-white/40">Números Tabulares</span>
                        <button 
                            onClick={() => updateDraft('useTabularNums', !draft.useTabularNums)}
                            className={`py-2 px-3 rounded-lg text-2xs font-black uppercase transition-all border ${draft.useTabularNums ? 'bg-[var(--theme-primary)] border-transparent text-white' : 'bg-white/5 border-white/10 text-white/40'}`}
                        >
