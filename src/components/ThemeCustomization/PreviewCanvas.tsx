...
export const PreviewCanvas: React.FC<{
    ...
    const { layoutDensity, setLayoutDensity } = useSarak();
        ...
        <div className="flex-grow flex flex-col relative overflow-hidden bg-[var(--theme-body)]/20">
            ...
            <div className="flex items-center justify-between shrink-0 relative z-30 p-4">
                ...
                <div className="flex-grow flex justify-center">
                    <div className="flex gap-4 items-center bg-[var(--theme-body)]/80 p-1.5 rounded-full border border-[var(--theme-border)]/50 backdrop-blur-md">
                        {[
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-full text-[11px] font-bold transition-all ${layoutDensity === preset.id
                                    ...
                                    : 'text-[var(--theme-muted)] hover:text-[var(--theme-title)]'} `}
                            >
                                {preset.label}
                            </button>
                        ))}
            ...
            <div className="flex-grow flex items-center justify-center p-4 relative z-10 overflow-auto custom-scrollbar">
                <motion.div
                    ...
                    className={`preview-container ${mode} layout-${previewLayoutId.replace('custom-', '')} bg-[var(--theme-body)] text-[var(--theme-main)] rounded-[40px] shadow-[0_50px_100px_rgba(0,0,0,0.3)] overflow-hidden flex flex-col relative transition-colors duration-500 ${config['--bg-texture'] ? `texture-${config['--bg-texture']}` : ''}`}
                >
                    ...
                    <div className="flex flex-grow overflow-hidden">
                        <div className="w-16 sm:w-20 bg-[var(--theme-card)]/30 border-r border-[var(--theme-border)] shrink-0 flex flex-col items-center py-6 gap-6">
                            ...
                            {[BarChart3, MessageSquare, History, Users, Shield, Database].map((Icon, i) => (
                                <div key={i} className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${i === 0 ? 'bg-[var(--theme-primary)]/10 text-[var(--theme-primary)]' : 'text-[var(--theme-muted)] hover:text-[var(--theme-title)] hover:bg-[var(--theme-body)]/50'}`}>
