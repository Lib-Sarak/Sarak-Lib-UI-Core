...
| --- | --- | --- |
| `npm run audit` | R1 R2 R3 R4 R5 R7 R8 R9 R10 R17 R23 R31 R32 | agrega 12 auditores (2 novos na `plan-12`, 1 novo na `plan-16`, 1 novo na `plan-24`); os limites de cada um estão abaixo |
...
| `audit/auditor_contraste.mjs` → `verify_contrast.ts` *(plan-24)* | R31 | wrapper fino (mesmo padrão de `auditor_presets.mjs`/`auditor_sectionpointers.mjs`); cobre só os 36 pares texto/fundo levantados e listados em `PAIRS` — não "todo par possível"; só resolve HEX/`rgb()`/`rgba()`/`transparent`, `hsl()`/`var()`/gradiente é declarado pulado, nunca chutado; ver `LIMITES DECLARADOS` no próprio arquivo |
