...
- [ ] O arquivo `theme_table_mapping.json` (no diretório `catalog/`) foi atualizado via comando? (Verifique lendo o arquivo gerado).
- [ ] O token foi inserido fisicamente na partição JSON correta dentro do Catálogo (`catalog/partitions/`)?
- [ ] O Script de verificação de paridade (1:1:1:1:1) foi aprovado com `Exit Code 0`?
