...
- [x] `auditor_typescript.mjs` = **0** em `src/**` (base inteira).
- [x] `run_audit.mjs` 100% verde (7/7 auditores).
- [x] `npx tsc --noEmit` = 0 erros; `npx vitest run` sem regressão.
- [x] Tabela de exceções `@ts-expect-error` desta spec preenchida (idealmente vazia) e justificada.
- [x] Índice §0.6 atualizado + Regra 1 da Spec 50 marcada satisfeita.

# 5. Plano de Testes (Quality Gate)
## Testes Unitários
- [x] **Deve** manter toda a suíte de caracterização das fatias 61–63 verde na varredura final (nenhuma regressão acumulada).
## Testes de Contrato (Tipos)
- [x] **Deve** confirmar, via `tsc --noEmit` global em 0 erros, que a base atingiu o estado **Zero Any** declarado como lei absoluta no índice (§0.1).
