...
- **✅ Fase 1a — Gerador evoluído:** `scripts/generate-token-types.ts` agora emite `SarakDesignTokens` (tradução §3.3) + `DesignTokenId = keyof SarakDesignTokens` (chaves citadas quando necessário; dedup first-wins). **Saída compacta (4 membros/linha)** para o arquivo gerado não disparar o Clean Code "arquivo gigantesco" (~89 linhas, não 317). **NÃO editar o gerado à mão; regenerar com `npx tsx scripts/generate-token-types.ts`.**
...
- **Fase 3 — Sweep dos consumidores (queda em bloco):**
  ...
  - ✅ **Design não-difícil (fechado via Spec 61 "Design leftover", −8):** `DesignScope.tsx` (`design: SarakDesignState`), `types.ts` (novo `SarakTokenValue`), `master-map.ts` (`getDefaultDesignState`→`Record<string,SarakTokenValue>`), `SarakBackgroundRenderer.tsx` (cast morto removido). 1 cast de seam `as SarakDesignState` no `getSeedConfig` do `useDesignManager`. `tsc` 0 · Design+Provider 47/47 · paridade ✅.
  - 🟥 **Resíduo difícil (deferido → tabela de exceções Spec 64):** `manifest.ts` (4) = mapa de callbacks heterogêneo (`transform?: (v) => …` com param/retorno diferentes por entrada; `unknown` quebra os `(v: string)` por contravariância, `never` quebra os testes que chamam com valores reais) · `color-engine`/`presets` (5: `color-engine.ts` 4 + `presets/modules/index.ts` 1 + `presets/components/cards.ts` 1, em `core/Design`) = mapa de token com índice dinâmico que cascateia para os previews em `features/`. Removê-los troca poucos `any` por muitos casts (net-negativo) — decisão própria na Spec 64/refino.
