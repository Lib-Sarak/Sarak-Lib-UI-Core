...
14. [ ] **[31 - Limpeza da Rodada 2 (desinstalar do ERP)](./31-limpeza-rodada2-erp.md)** — pré-requisito do re-Selo: remove a instalação COMPLETA da rodada 1 da **raiz** do ERP (`src/`, `node_modules/`, `dist/`, `index.html`, `vite.config.ts`, `tsconfig*.json`, `database.sqlite`, `package.json`+lock, skills de UI em `.agents` **e** `.claude`). ⚠️ **Não reusar a Spec 25** — o inventário dela aponta para `frontend/`, pasta que não existe mais (a rodada 1 instalou na raiz). 5 decisões HITL. Prompt **P14**.
...
16. [ ] **[30 - Polimento pós-Selo](./30-polimento-pos-selo.md)** — achados 6/7/8 (baixa prioridade, NÃO bloqueia o Selo, roda DEPOIS do re-Selo): chave natural do `renderFor` (`hash`), code-splitting do bundle (baseline 3,9 MB / 992 KB gzip), warning de `input[type=color]` com `var(...)`. Prompt **P16**.
