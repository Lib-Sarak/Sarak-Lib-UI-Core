...
> **Prompts prontos:** `specs/plan/00-prompts-execucao.md` contém um prompt autocontido para cada item **ainda pendente**, numa conversa nova. Desde a limpeza de 2026-07-21 ele guarda **só o que falta executar** — hoje **P15** (condicional), **P17**, **P18** e **P19**, numerados conforme o item do roteiro abaixo (por isso há lacunas). Os prompts dos itens já concluídos foram removidos; o histórico está no git e no `00-progresso.md`.
...
18. [ ] **[40 - Teste Real](./40-teste-real.md)** — **2ª parte do teste em consumidor real:** implementar as funcionalidades REAIS do ERP (Propostas/Contratos/Projetos) com conexões REAIS (Supabase do ERP), **100% via manifesto**. Regra de ouro: no importador **só o `manifest.json` muda**; qualquer lacuna da lib é corrigida **NA FONTE**, nunca adaptada no ERP (fix-at-source iterativo, diferente da Spec 26 que só mede). Prova que a lib sustenta produção, não só demo. Prompt **P18**.
19. [ ] **[41 - Piso de Bundle: barris de ícone](./41-piso-de-bundle-barris-de-icone.md)** — *origem: achado da execução da Spec 30*. A Spec 30 provou que `manualChunks` **não reduz bundle num renderizador de manifesto** (o Registry precisa de todo componente não-lazy em runtime — despacho por string derrota a análise estática). A causa atacável é outra: 6 arquivos fazem `import * as LucideIcons` com **acesso por índice dinâmico**, o que impede tree-shaking e arrasta ~1500 ícones; os 5 cards ainda **burlam** o átomo oficial `SarakIcon`/`IconMap` curado. **MEDIR ANTES de refatorar** (conclusão negativa também é entrega); documentar nomes de ícone válidos no catálogo (hoje é a exceção da regra dura de tokens). ⚠️ **Executar ANTES da 42** — 2 arquivos em comum, nunca em paralelo. Prompt **P19**.
20. [ ] **[42 - Generalizar SarakCoreCard / SarakCardGrid](./42-generalizar-cardgrid-corecard.md)** — *follow-up da Spec 30 (decisão HITL de 2026-07-21)*: `SarakCoreCard` (variante `"classic"`, DEFAULT de `SarakCardGrid`) tem o mesmo domínio de catálogo de modelos LLM que o `SarakActionCard` tinha ("Custo In/Out", "Janela de Contexto" com aritmética embutida); e `SarakCardGridProps.mapping` publica `price_in`/`price_out`/`context` no TIPO — remover é breaking change de contrato tipado, exige nota de migração e teste de caracterização do `SarakCoreCard` ANTES do refactor. Não bloqueia nenhum Selo. ⚠️ Rodar **depois** da 41 (arquivos em comum); se rodar antes, preservar a troca de ícone dela. Prompt **P20**.
