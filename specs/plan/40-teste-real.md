...
status: "🟢 APROVADO (2026-07-25) — Teste Real CONCLUÍDO: todas as rodadas do ciclo 40.x (40.1–40.4) executadas e validadas pelo dono; R1–R12 verdes (R12/hambúrguer fechado pela correção P21.3-C: detecção real testada + prova headless nos 3 viewports). LIBERA a Spec 46."
prioridade: "Máxima"
...
### Rodada 3 (Spec 40.3) executada — 2026-07-25
A validação prática do dono após a 40.2 confirmou o cromo, mas apontou o **gap central**: a adaptação multidispositivo era **pontual** (só `SarakDataTable`→cards). Esta rodada tornou o **layout multidispositivo abrangente e por padrão (zero-config)** — fix de lib, o ERP herda sem código:
- **Cromo reflui por dispositivo** (L1) → `SarakAppChrome` agora adapta via `useSarakDevice`: desktop = sidebar/topbar; **tablet** = topbar compacta (a sidebar de 240px comeria a tela ≤1024px); **celular** = a nav COLAPSA atrás de um **hambúrguer → drawer** (`SarakAppChromeMobile`), não come a tela, acessível (`aria-expanded`/foco/ESC/scrim, foco devolvido ao toggle via `useFocusTrap`). Decisão de design resolvida deterministicamente pela árvore da lib (drawer = convencional, genérico p/ qualquer nº de itens e `navigationStyle`) — sem HITL.
- **Primitivas de layout mobile-first** (L2) → `SarakGrid` colapsa um `templateColumns` fixo para 1 coluna no celular (nunca estoura) e aceita `ResponsiveValue`; `SarakFlex` quebra por padrão (`wrap`) e aceita `ResponsiveValue` em `direction`; `SarakSplitPane` empilha no celular. Helper puro `resolveResponsiveValue` exposto.
- **Densos que a lib usa** (L3) → `SarakTable` também colapsa para cards no celular (`SarakTableCards`), reusando colunas/rótulos. `SarakManagementGrid`/`SarakDataGrid` ficam registrados (o ERP não os usa — as telas migraram para cards/listas).
- **Contrato documentado** (L4) → `docs/temas-cromo-e-multidispositivo.md` §4: matriz do que adapta automático por dispositivo + onde refinar com `ResponsiveValue`.

**Estado após a 40.3 (+ correção P21.3-C):** L2/L3/L4 (primitivas mobile-first, densos, contrato) **code-complete e com gates verdes**. O **L1** REPROVOU na 1ª validação de browser do dono (cromo não colapsava em hambúrguer no celular), mas a correção P21.3-C mostrou que **NÃO era bug de código — era BUILD STALE**: o ERP rodava uma cópia velha do `@sarak/lib-ui-core` no pnpm store (07-24, sem o cromo 40.3); como a `version` ficou `3.0.0`, o `pnpm install` do `file:` não recopiou a dist nova (cache mais fundo que o `.vite`). **Prova headless (Playwright/Chromium) no build GENUINAMENTE fresco:** nos 3 viewports o cromo reflui — **375px = hambúrguer + drawer** (abre/fecha por ESC), 768px = topbar compacta, 1280px = topbar. A detecção foi **endurecida** (`useSarakDevice` self-contained: estado inicial já do viewport, sem flash de desktop; imune a fragmentação de contexto/chunk) e ganhou **testes do caminho real** (sem `overrideDevice`). Gates: lib **305 arq·975 testes**; `run_audit` no **baseline exato**; catalog/barrel/build+DTS/package verdes. **Achado ortogonal do mesmo browser:** avisos `[Sarak:Design] token fora do contrato — descartado` → **Spec 40.4** (reconciliação do contrato de tokens). *(A "kit de uso do consumidor" foi renumerada de 40.4 → **Spec 50**.)* ✅ **Teste Real APROVADO pelo dono (2026-07-25)** — todas as rodadas do ciclo 40.x fechadas; R1–R12 verdes; **libera a Spec 46**.

### Rodada 4 (Spec 40.4) executada — 2026-07-25
O achado ortogonal da 40.3 (avisos `[Sarak:Design] Token "X" com valor fora do contrato — descartado`) era **drift interno da lib**: o tema COMPLETO (defaults + tema shippado) carregava valores que o validador estrito (Spec 44) rejeitava, dropando um bloco de eixos silenciosamente — contradizendo o R10. Auditoria EXAUSTIVA (`auditTokenContract`, nova função pura em `validation.ts`, reusa o mesmo `coerceTokenValue` do runtime) rodada sobre **defaults + os 18 `GLOBAL_THEMES` + os 102 presets parciais de componente** — não só a amostra de 9 tokens que o console mostrou no boot, mas **117 violações em 21 tokens distintos**. Reconciliação **na fonte, token a token** (nunca afrouxando `validateDesign`): 8 tokens ganharam opção de enum legítima e faltante (`easeMain`/`easeOut` — linear + 2 curvas exclusivas; `h1Weight`/`h2Weight` — 800/900; `chartType` — area; `shadowOrientation` — diagonal; `layout` — masonry; `maxContentWidth` — 1000px; `globalBackgroundBlendMode` — color-dodge); os demais (`btnStyleType`/`cardVariant`/`switchStyleType`/`surfaceMaterial`/`systemTone`/`shadowColorMode`/`searchPositionSidebar`/`bodySize`/`iconFamily`/`colorPalette`/`cardHoverStyle`/`flowGridStyle`) tiveram o VALOR do tema corrigido para uma opção canônica já existente, preservando a intenção visual (ex.: `minimalist-airy.systemTone: 'neutral'→'light'`, coerente com `mode:'light'`; `industrial-dashboard.systemTone→'tactical'`, coerente com a descrição "HUD/painel técnico"). Paridade 1:1:1:1:1:1 intacta (`verify_parity.ts`: 409/409/409) — só se adicionaram OPÇÕES a tokens existentes, nenhum token novo.

**Estado após a 40.4:** **code-complete e com gates verdes** — gate anti-regressão permanente (`tokenContractParity.test.ts`, na suíte) falha o build se qualquer default/tema/preset shippado cair fora do próprio contrato; teste de console limpo (`shippedThemesConsoleClean.test.ts`) confirma **zero aviso `fora do contrato`** carregando cada um dos 18 temas shippados; caracterização do `minimalist-airy` prova que os 9 eixos antes descartados agora aplicam e que nenhum outro token do tema regrediu. Suíte completa **307 arq · 996 testes** (era 305/975 — +2 arquivos/+21 testes, 0 regressão); `catalog:check`/`barrel:check`/`build`+DTS/`package:check` verdes; `run_audit.mjs` no **baseline exato** (1 hardcode + 3 ghostvars + 3 órfãos de manifesto). **Achado colateral registrado (fora de escopo, não corrigido aqui):** o `PreviewCanvas` (Canvas do Design Engine) renderiza o `design` de um tema RAW, sem passar por `validateDesign` — por isso valores fora do contrato chegavam a virar CSS Variable ali (diferente do boot real via `useDesignManager`, que sempre valida); o snapshot deste teste foi atualizado para refletir os valores corrigidos. Falta só a **confirmação do dono no browser** (console sem `fora do contrato` no ERP; tema renderiza igual ou melhor).
...
| R10 | **A config INTEIRA do Design Engine propaga** (não só `accentColor`): cor **E fonte E cromo topbar/sidebar E raio** repintam em TODAS as telas; abas não quebram; **todas as funcionalidades da lib alcançáveis** pelo consumidor (barril completo, composição granular, multi-dispositivo) | 🟢 **Confirmado no browser (Spec 40.1 + 40.2 + 40.4)** — cromo idêntico-por-app com ícones (4 apps) e densos mobile-usáveis; **Spec 40.4** fechou o bloco de eixos que era silenciosamente descartado por drift de contrato (21 tokens reconciliados, zero aviso `fora do contrato` nos 18 temas shippados) — agora é literal e ✅ **confirmado no browser pelo dono (2026-07-25) + commitado**; o único resíduo do eixo multidispositivo é o hambúrguer, rastreado no R12 |
...
| R12 | **Layout multidispositivo por padrão** (Spec 40.3): o cromo reflui por dispositivo (celular = nav atrás de hambúrguer/drawer, não come a tela, acessível); primitivas de layout mobile-first (`SarakGrid`/`SarakFlex`/`SarakSplitPane`) + `ResponsiveValue`; densos que a lib usa colapsam no celular; contrato documentado; ERP herda zero-config | 🟡 **Code-complete + provado headless (Spec 40.3 + correção P21.3-C):** L1 REPROVOU no 1º browser do dono por **BUILD STALE** (cópia velha do `@sarak/lib-ui-core` no pnpm store; `version` 3.0.0 imóvel → `file:` não recopiou), não por bug. **Prova headless no build fresco:** hambúrguer+drawer a 375px, topbar a 768/1280. Detecção endurecida (self-contained, sem flash) + testes do caminho real. Lib 305/975, audit no baseline, ERP `tsc` verde. ✅ **APROVADO pelo dono (2026-07-25) — R12 fechado.** |
