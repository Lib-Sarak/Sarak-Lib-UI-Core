...
**Estado após a 40.3 (+ correção P21.3-C):** L2/L3/L4 (primitivas mobile-first, densos, contrato) **code-complete e com gates verdes**. O **L1** REPROVOU na 1ª validação de browser do dono (cromo não colapsava em hambúrguer no celular), mas a correção P21.3-C mostrou que **NÃO era bug de código — era BUILD STALE**: o ERP rodava uma cópia velha do `@sarak/lib-ui-core` no pnpm store (07-24, sem o cromo 40.3); como a `version` ficou `3.0.0`, o `pnpm install` do `file:` não recopiou a dist nova (cache mais fundo que o `.vite`). **Prova headless (Playwright/Chromium) no build GENUINAMENTE fresco:** nos 3 viewports o cromo reflui — **375px = hambúrguer + drawer** (abre/fecha por ESC), 768px = topbar compacta, 1280px = topbar. A detecção foi **endurecida** (`useSarakDevice` self-contained: estado inicial já do viewport, sem flash de desktop; imune a fragmentação de contexto/chunk) e ganhou **testes do caminho real** (sem `overrideDevice`). Gates: lib **305 arq·975 testes**; `run_audit` no **baseline exato**; catalog/barrel/build+DTS/package verdes. **Achado ortogonal do mesmo browser:** avisos `[Sarak:Design] token fora do contrato — descartado` → **Spec 40.4** (reconciliação do contrato de tokens). *(A "kit de uso do consumidor" foi renumerada de 40.4 → **Spec 50**.)* Falta só a **reconfirmação do dono no browser com o build fresco** (força o refresh da cópia stale — ver procedimento no `00-progresso`).
...
| R10 | **A config INTEIRA do Design Engine propaga** (não só `accentColor`): cor **E fonte E cromo topbar/sidebar E raio** repintam em TODAS as telas; abas não quebram; **todas as funcionalidades da lib alcançáveis** pelo consumidor (barril completo, composição granular, multi-dispositivo) | 🟡 **Code-complete (Spec 40.1 + 40.2 + 40.4)** — cromo idêntico-por-app com ícones (4 apps) e densos mobile-usáveis; **Spec 40.4** fechou o bloco de eixos que era silenciosamente descartado por drift de contrato (21 tokens reconciliados, zero aviso `fora do contrato` nos 18 temas shippados) — agora é literal, não só code-complete parcial; falta a prova de browser do dono |
...
| R12 | **Layout multidispositivo por padrão** (Spec 40.3): o cromo reflui por dispositivo (celular = nav atrás de hambúrguer/drawer, não come a tela, acessível); primitivas de layout mobile-first (`SarakGrid`/`SarakFlex`/`SarakSplitPane`) + `ResponsiveValue`; densos que a lib usa colapsam no celular; contrato documentado; ERP herda zero-config | 🟡 **Code-complete + provado headless (Spec 40.3 + correção P21.3-C):** L1 REPROVOU no 1º browser do dono por **BUILD STALE** (cópia velha do `@sarak/lib-ui-core` no pnpm store; `version` 3.0.0 imóvel → `file:` não recopiou), não por bug. **Prova headless no build fresco:** hambúrguer+drawer a 375px, topbar a 768/1280. Detecção endurecida (self-contained, sem flash) + testes do caminho real. Lib 305/975, audit no baseline, ERP `tsc` verde. Falta só a reconfirmação do dono no browser fresco. |
