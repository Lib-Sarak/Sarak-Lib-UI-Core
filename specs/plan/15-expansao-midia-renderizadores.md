...
- [ ] O componente Markdown não injeta HTML cru malicioso (*Sanitization* via purificador seguro embutido).
- [ ] O visualizador de PDF possui barra de controles customizada na parte superior (Botão de Zoom In, Zoom Out e Download) alinhada à estética da Sarak UI.
- [ ] Tabelas escritas em sintaxe de Markdown no JSON devem se transmutar para tabelas HTML formatadas.

# 4. Plano de Testes (Quality Gate)

## Testes Unitários
- [ ] **Deve** higienizar e neutralizar com sucesso uma string de ataque contendo injeção `javascript:alert(1)` vinda da string do Markdown.
- [ ] **Deve** injetar o CSS da sintaxe (highlighting de código fonte) perfeitamente de acordo com se o ambiente estiver em *Dark Mode* ou *Light Mode*.

## Testes de Contrato (API)
- [ ] N/A.

## Testes E2E (Integração)
- [ ] Visual: Confirmar a não deformação visual quando uma imagem muito larga for inserida no renderizador Markdown, assegurando uso de `max-width: 100%`.
