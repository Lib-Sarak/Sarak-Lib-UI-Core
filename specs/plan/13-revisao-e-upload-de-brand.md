...
- **Mapeamento Direto:** os tokens reais são `primaryColor`/`secondaryColor` (`src/core/Provider/types.ts:84-85`) — **não `colorPrimary`/`colorSecondary`**, que não existem no schema (correção da spec original). Sobrescrever esses dois já propaga para botões/links/estados ativos de toda a biblioteca (é assim que o motor de temas já funciona hoje).
- **Geração de Escala Automática — já existe, não precisa ser criada, mas hoje é interna.** `computeColorVariants(v: string, fallback: string)` (`src/core/Provider/utils/color-engine.ts:153`) já calcula variantes claras/escuras a partir de 1 cor base. **Correção:** ela **não** é exportada publicamente por `src/index.ts` (o barrel do pacote `@sarak/lib-ui-core`) — este só faz `export { default as SarakUIProvider, useSarakUI } from './core/Provider/SarakUIProvider'` (export nomeado, não `export *`), então mesmo `SarakUIProvider.tsx:21` reexportando `computeColorVariants` internamente, isso não se propaga até o barrel público. Hoje só `manifest.ts`, `useDesignVariables.ts` e o próprio `SarakUIProvider.tsx` a consomem. **Isso não bloqueia esta spec:** a UI de "Brand & Identidade" descrita aqui é uma feature interna do próprio pacote (`src/features/DesignEngine/...`), então pode importar `computeColorVariants` diretamente de `core/Provider/utils/color-engine` (caminho interno) sem precisar mexer no barrel. Só será necessário adicionar `export { computeColorVariants } from './core/Provider/utils/color-engine'` a `src/index.ts` se, no futuro, um consumidor externo (fora deste pacote) precisar chamar essa função diretamente — não é o caso do fluxo descrito nesta seção.
