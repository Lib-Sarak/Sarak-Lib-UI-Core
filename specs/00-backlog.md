...
| 12 | `SarakAppChrome.tsx` está em 249 linhas por `wc -l` e **250 pela contagem do gate** — exatamente no teto da R9 (`auditor_cleancode` conta pela API do TS, um a mais que `wc -l` quando o arquivo termina em nova linha). As duas próximas plans do cromo editam esse arquivo de forma substancial: a primeira linha nova o reprova, então as duas precisam extrair antes de acrescentar | ritual | 2026-09-09 | médio |
| 13 | Os prompts de execução em paralelo não avisam que o worktree contém trabalho de outras plans — um executor propôs `git checkout --` em ~35 arquivos alheios, achando que eram efeito colateral próprio, e teria apagado entregas não commitadas de três plans. A disjunção por arquivo protege o veredito do revisor, não a percepção do executor | ritual | 2026-09-09 | alto |
