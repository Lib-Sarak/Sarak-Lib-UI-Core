...
# 5. Extensão da Camada 6: Presets Companion e Exceções Internas
Quando o hook controlador principal (`useStructuralStyles.ts`) cresce além do limiar de Clean Code por acumular variações de grid/espaçamento responsivo sem token 1:1 equivalente no catálogo, a solução arquitetural correta **não é criar hardcode nem um carve-out permanente no auditor**: é extrair essas variações para um **arquivo companion de presets nomeados** (ex: `useStructuralStyles.presets.ts`, contendo `RESPONSIVE_GRID_PRESETS`/`RESPONSIVE_SPACING_PRESETS`), mantendo o hook principal magro e a classe Tailwind responsiva na camada `.ts` do hook — fora do arquivo `.tsx` que o `auditor_hardcoded.mjs` varre.

**Exceção arquitetural documentada:** componentes de `src/.../internal/` deliberadamente desacoplados da árvore do `SarakUIProvider` (ex: `CalendarPanel`) podem aplicar estilos estruturais (ex: `gridTemplateColumns`) diretamente via `style` inline, sem passar pelo hook controlador — pois não têm acesso ao contexto de tema por design, não por omissão.
