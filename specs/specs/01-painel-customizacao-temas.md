...
# 2. Regras de Negócio
- **Regra 0 (Alcance via Manifesto — paridade plug-and-play):** o painel completo (`CustomizationPanel`) é um `type` NATIVO do Manifest Engine (registrado no `NATIVE_COMPONENTS` via `React.lazy`). Um consumidor 100% declarativo o entrega com `{ "type": "CustomizationPanel" }` numa rota do `shell`/`routes` — sem escrever nenhum componente React. Os ids legados do Discovery (`mx-customization`/`personalization`) permanecem para o shell antigo (Spec 04); a equivalência entre os dois caminhos é cobrada pelo gate `RegistryParity.test.tsx`.
