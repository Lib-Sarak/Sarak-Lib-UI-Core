...
- **Regra 5 (Controlador de Variantes Dinâmicas):** É ESTRITAMENTE PROIBIDO escrever funções condicionais (ex: `getButtonStyles()`), estruturas longas de `switch/case`, ou injeção de `<style>` no JSX para rotear variantes de design complexas (como `neon`, `frosted`). Toda abstração de UI complexa que envolva variantes dinâmicas DEVE ser processada e mapeada externamente por um **Hook Puro Controlador** (ex: `useAtomicStyles`). Os componentes React atômicos são *"Dumb Components"*, seu código JSX não pode ser sujo com lógica de processamento visual.
