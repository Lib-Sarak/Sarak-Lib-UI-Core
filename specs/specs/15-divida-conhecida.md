...
| 43 | **O piso de cobertura tem 4,5 pontos de folga — e a cobertura não é estável entre execuções.** Medido pelo revisor em 2026-08-18, suíte 100% verde (317 arquivos / 1371 testes): `lines` em **75,97%** contra piso commitado de **71,47%**. O gate imprime `MELHOROU (nada bloqueado)` e **pede** o `--write`, que ninguém rodou — as plans 47/49/50 melhoraram o número e foram sintetizadas sem regravar o piso, contra o que a **R20** manda (*regravar e commitar **junto do conserto***). Consequência: **qualquer regressão que caiba nos 4,5 pontos não bloqueia nada** — é o mesmo *"teto folgado é um gate desligado pela metade"* de [[01-gates-e-baseline]] §4.2, invertido para piso. ⚠️ **E o número oscila:** o executor da `plan-51` mediu **76,02%** (`statements` 74,25%) e o revisor mediu **75,97%** (`statements` 74,19%) **no mesmo commit**, com as duas suítes verdes — então a cobertura varia por execução, o que é candidato natural aos **três estados mutáveis de módulo** que a `plan-46` §2.3 nomeia (ordem de execução muda quais ramos são exercitados). | `gates/baselines/coverage-floor.json` (piso) · `gates/scripts/release/check-coverage-floor.mjs` (o gate) | **R8.1** (piso móvel) · **R20** (baseline não regride) | **Decidir DEPOIS da `plan-46`** — não antes. Piso levantado a partir de uma suíte não-determinística bloqueia na execução ruim, que é a armadilha que [[01-gates-e-baseline]] §6 proíbe. Se a `plan-46` **consertar** a intermitência, uma medição basta; se **declarar** como dívida, o piso tem de ser o **mínimo sobre N execuções**, nunca uma só. É essa dependência que impede a folga de virar plan própria hoje |
