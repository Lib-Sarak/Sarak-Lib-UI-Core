...
> Restam **18, 23, 26**, mais o **45**, acrescentado em 2026-08-18.
...
| 26 | **Automação que exercite um `install` de verdade.** Confirmado: **0 ocorrências** de `child_process`/`execSync` nos testes de `bin/scaffold/`. As provas de npm/pnpm/yarn foram feitas à mão, uma vez. Idem o `check --notify` do `predev` | — | Nenhuma regra escrita. Depende de CI (`plan-05`) e foi roteado para o ciclo de instalação/atualização (`plan-10`) |
| 45 | **E2E e regressão visual em navegador real.** Capacidade que **existiu e foi removida** em 2026-08-18 (decisão do dono, tomada duas vezes) por produzir **verde falso** — nenhum pipeline a executava. Cobria: o não-vazamento do modo embarcado medido em CSS renderizado (**R24**), o boot do painel pintado num browser, `var()` resolvendo de fato, e regressão visual de 8 componentes. **Fica adiada até existir CI** (`plan-05`) — é ela que dá a um aparato de browser um lugar para rodar. ⚠️ **Perda transversal:** a base ficou **sem nenhuma ferramenta** de medir CSS/`var()` em navegador, o que alcança qualquer aceite futuro que dependa disso ([[11-testes-e-cobertura]] §7.2) | — *(a capacidade não existe; o vão é a ausência dela)* | **R24**, que por isso é **⚠️** e não ✅ — os dois gates restantes provam estrutura, não CSS renderizado | Depende da **`plan-05`** (CI). Enquanto não houver onde rodar, reinstalar o aparato só reproduziria o verde falso que motivou removê-lo |
