...
class AgentValidator {
     ...
     */
    static validateAgentAssets(agentId, basePath) {
        logger_1.logger.debug(`[LINTER] Starting validation for agent: '${agentId}' at path: ${basePath}`);
        ...
        if (fs.existsSync(defaultsPath)) {
            ...
            try {
                ...
                logger_1.logger.error(`[LINTER] JSON Syntax Error in global defaults.json: ${e.message}`);
        ...
        try {
            ...
            logger_1.logger.error(`[LINTER] JSON Syntax Error in config.json for agent '${agentId}': ${e.message}`);
        ...
        if (fs.existsSync(manifestPath)) {
            ...
            try {
                ...
                logger_1.logger.error(`[LINTER] JSON Syntax Error in manifest.json for agent '${agentId}': ${e.message}`);
        ...
        logger_1.logger.info(`[LINTER] Agent '${agentId}' successfully passed all validation checks.`);
    }
    static _validateConfigStructure(config, agentId) {
        // Required Keys
        ...
        if ('triggers' in config) {
            ...
            for (const [triggerName, rule] of Object.entries(triggers)) {
                ...
                if ('fields' in rule) {
                    if (!Array.isArray(rule.fields) || !rule.fields.every((f) => typeof f === 'string')) {
    ...
    static _validateManifestStructure(manifest, agentId) {
    ...
    static _validateDefaultsStructure(defaults) {
        if ('llm' in defaults) {
